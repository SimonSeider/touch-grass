...
float perlinNoise3D(vec3 pIn, float frequency, int octaveCount) {
  float octaveFreqFactor = 2.0;
  float sum = 0.0;
  float weightSum = 0.0;
  float weight = 0.5;
  for (int oct = 0; oct < octaveCount; oct++) {
    vec4 p = vec4(pIn.x, pIn.y, pIn.z, 0.0) * frequency;
    float val = glmPerlin4D(p, vec4(frequency));
    sum += val * weight;
    weightSum += weight;
    weight *= weight;
    frequency *= octaveFreqFactor;
  }
...
float remap(float v, float iMin, float iMax, float nMin, float nMax) {
  return nMin + (((v - iMin) / (iMax - iMin)) * (nMax - nMin));
}
...
vec4 stackable3DNoise(vec3 coord) {
  ...
  float worleyFBM0 = worleyNoise1 * 0.625 + worleyNoise2 * 0.25 + worleyNoise3 * 0.125;
  float worleyFBM1 = worleyNoise2 * 0.625 + worleyNoise3 * 0.25 + worleyNoise4 * 0.125;
  float worleyFBM2 = worleyNoise3 * 0.75 + worleyNoise4 * 0.25;
